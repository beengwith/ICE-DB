...
Go
Create Function [dbo].[GetTrackDate] (
    @a_time DateTime,
    @delay_hrs int=8
)
Returns DateTime
...
As Begin
    ...
    Set @end_time = dateadd(hh, @delay_hrs, dbo.GetTrackDate(@a_time, @delay_hrs));
    Return @end_time;
End
...
As Begin
    Declare @end_time DateTime;
    Set @end_time = dateadd(hh, 24 + @delay_hrs, dbo.GetTrackDate(@a_time, @delay_hrs))
...
AS Begin
    Return (
        CASE WHEN DateDiff(
            dd,
            dbo.GetTrackDate(@time1, @delay_hrs),
...
Go
Create Function [dbo].[GenerateInOutID] (
    @eid varchar(10)
) Returns Int
As Begin
    Declare @InOutId int;
...
Create Procedure [dbo].[CreateOrUpdateDailyAttendance] (
    ...
    @timeStatus Varchar(50)
) As Begin

-- Check if an entry already exists
Declare @prevAttStatus Varchar(50);
Declare @prevTimeStatus Varchar(50);
Select
    @prevAttStatus=AttendanceStatus,
    @prevTimeStatus=TimeStatus
From
    EmployeeAttendance As Ea
    Inner Join Employee as Emp on Ea.EmployeeId = Emp.EmployeeId
Where
    Emp.EmployeeId = @eid
    And AttendanceDate = @trackDate

If @prevAttStatus is NULL Begin
    -- Insert if there is no existing entry
    Insert Into
    EmployeeAttendance(AttendanceDate, EmployeeID, AttendanceStatus,
        TimeStatus)
    Values(@trackDate, @eid, @attendanceStatus, @timeStatus)
End
Else Begin
...
Declare @lastInOutID Int;
Declare @lastTrackDate DateTime;
Declare @lastInOutTime DateTime;
Declare @LastInOutStatus Varchar(10);
...
Else Begin
    ...
    If @lastInOutStatus = 'In' Begin
        ...
        If @lastTrackDate < @trackDate Begin
            ...
            End
            Exec CreateOrUpdateDailyAttendance @eid, @dateCounter, 'P', @timeStatus;
        End
        -- Detail Entry
        Insert INTO
    ...
    Else Begin
        ...
        If @lastTrackDate < @trackDate Begin
            ...
            While @dateCounter < @trackDate Begin
                ...
                Exec CreateOrUpdateDailyAttendance @eid, @dateCounter, @attendance_status, '';
                Set @dateCounter = DateAdd(dd, 1, @dateCounter);
            End
            Exec CreateOrUpdateDailyAttendance @eid, @dateCounter, 'P', @timeStatus;
        End
...
DECLARE @inOutTime DateTime;
DECLARE @newInOutTime DateTime;
...
Select
    @lastDate=TrackDate,
    @lastInOutTime=InOutTime,
    @lastStatus=InOutStatus
From
    AttendanceDetails
Where ADID = (
        Select IsNull(Max(ADID),0)
...
Else Begin -- "In"
    ...
    Else Begin
        ...
        if dbo.IsEarlierWorkingDay(@lastDate, @trackDate, default) = 1 Begin
            ...
            if dbo.isLaterWorkingDay(@newInOutTime, @inOutTime, default) = 1 Begin
                Set @newInOutTime = dbo.GetWorkingDayEnd(@newInOutTime, default);
            End
            EXEC RecordAttendance @ecode, @lastDate, @newInOutTime;
            EXEC RecordAttendance @ecode, @trackDate, @inOutTime;
        End
