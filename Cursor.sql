...
IF (SELECT Count(*) FROM Ice_Project_Directory.dbo.UNISEntries ) != 0
BEGIN 
	SELECT TOP(1) @lastdate=C_Date, @lasttime=C_Time
	FROM Ice_Project_Directory.dbo.UNISEntries 
...
DECLARE eCursor CURSOR FOR
	...
	WHERE C_Date >= @lastdate
			 AND CAST(C_Time AS INT) > @lasttime
